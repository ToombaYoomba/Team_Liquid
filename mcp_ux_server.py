...
def _analyze_hits_data(df: pd.DataFrame) -> dict:
    ...
    if "ym:pv:URL" in df.columns:
        ...
        category_analysis = (
            ...
            .agg({"ym:pv:watchID": "count"})
            .rename(columns={"ym:pv:watchID": "views"})
            .reset_index()
        )

        category_analysis["percentage"] = (
            category_analysis["views"] / len(df) * 100
        ).round(2)
        analysis["url_category_analysis"] = category_analysis.to_dict("records")

        url_analysis = (
            df.groupby("ym:pv:URL")
            .agg({"ym:pv:watchID": "count"})
            .rename(columns={"ym:pv:watchID": "views"})
            .reset_index()
        )
...
def _analyze_visits_data(df: pd.DataFrame) -> dict:
    ...
    if "ym:s:watchIDs" in df.columns:
        ...
        session_metrics = {
            ...
            "depth_distribution": df["session_depth"]
            .value_counts()
            .sort_index()
            .to_dict(),
        }
...
def _load_combined_ux_data(hits_file: str, visits_file: str) -> str:
    ...
    combined_metrics = {
        "hits_per_visit": hits_analysis["total_hits"] / visits_analysis["total_visits"]
        if visits_analysis["total_visits"] > 0
        else 0,
        "total_data_points": hits_analysis["total_hits"]
        + visits_analysis["total_visits"],
    }
